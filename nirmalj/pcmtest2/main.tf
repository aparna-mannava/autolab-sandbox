...
locals {
  product     = "pcm"
  environment = "feature_CLOUD_100728_PCM"
  datacenter  = "ny2"
  facts                    = {
    "bt_product"           = "pcm"
    "bt_tier"              = "qa"
...
module "oradb_server_1" {
  ...
  hostname             = "us01vltdpcm12"
  alias                = "${local.product}-${local.facts.bt_tier}${local.facts.bt_env}-${local.facts.bt_customer}-db12"
...
module "oradb_server_2" {
  source               = "git::https://us-pr-stash.saas-p.com/scm/trrfrm/terraform-module-infrastructure.git?ref=master"
  hostname             = "us01vlgapcm12"
  alias                = "${local.product}-GA${local.facts.bt_env}-${local.facts.bt_customer}-db12"
  bt_infra_cluster     = "ny5-azc-ntnx-16"
  bt_infra_network     = "ny2-autolab-app-ahv"
  cpus                 = "4"
  memory               = "8192"
