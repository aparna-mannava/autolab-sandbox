...
locals {
  product     = "cfrm"
  environment = "feature_CLOUD_106748"
  datacenter  = "ny2"
  facts       = {
    "bt_product"         = "cfrm"
    "bt_customer"        = "bfs"
  ...
  db01facts    = {
    ...
    "bt_alias" = "${local.facts.bt_product}-${local.facts.bt_customer}-${local.facts.bt_tier}${local.facts.bt_env}-db84-${local.facts.bt_deployment_mode}"
  }
}
