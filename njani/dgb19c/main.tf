...
locals {
  lob         = "dgb"
  product     = "dgb"
  environment = "feature_DGBDO_15718"
  datacenter  = "ny2"
  facts         = {
    "bt_customer"         = "fi7799" #ex: fiXXXXXX
...
module "cloud_dbserver_1" {
  ...
  hostname             = "us01vlautodb87"
  alias                = "${local.lob}-${local.facts.bt_tier}${local.facts.bt_env}-${local.facts.bt_customer}-db87"
