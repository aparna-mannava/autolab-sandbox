...
locals {
  lob         = "dgb"
  product     = "dgb"
  environment = "feature_CLOUD_106894"
  datacenter  = "ny2"
  facts         = {
    "bt_customer"         = "fi9999" #ex: fiXXXXXX
...
module "cloud_dbserver_1" {
  ...
  hostname             = "us01vlautodb89"
  alias                = "${local.lob}-${local.facts.bt_tier}${local.facts.bt_env}-${local.facts.bt_customer}-db89"
