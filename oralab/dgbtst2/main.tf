...
module "dblab_1" {
  ...
  hostname             = "us01vldbtst5"
  alias                = "${local.facts.bt_product}-${local.facts.bt_tier}-oralb05"
...
module "dblab_2" {
  source               = "git::https://us-pr-stash.saas-p.com/scm/trrfrm/terraform-module-infrastructure.git?ref=master"
  hostname             = "us01vldbtst6"
  alias                = "${local.facts.bt_product}-${local.facts.bt_tier}-oralb06"
  bt_infra_cluster     = local.facts.bt_infra_cluster
  bt_infra_network     = local.facts.bt_infra_network
  lob                  = local.facts.bt_lob
  foreman_environment  = local.facts.environment
...
module "dblab_3" {
  source               = "git::https://us-pr-stash.saas-p.com/scm/trrfrm/terraform-module-infrastructure.git?ref=master"
  hostname             = "us01vldbtst7"
  alias                = "${local.facts.bt_product}-${local.facts.bt_tier}-oralb07"
  bt_infra_cluster     = local.facts.bt_infra_cluster
  bt_infra_network     = local.facts.bt_infra_network
  lob                  = local.facts.bt_lob
  foreman_environment  = local.facts.environment
